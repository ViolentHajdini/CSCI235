...
{
    Node<T>* first_ptr = head_ptr_;//copy the head_ptr to first_ptr so a new head_ptr can be assigned.
    if(getLength()>=2)//so if the list contains nothing or one item the invert function doesn't break
    {
        invertRest(head_ptr_->getNext());//calls recursive function using the second node
        first_ptr->getNext()->setNext(first_ptr);//after recursive calls set the former second nodes pointer to the first_ptr or the former headpointer.
        first_ptr->setNext(nullptr);//set first pointer next to null pointer so it is not longer pointing at former second node.
    }
...
{
    if(current_first_ptr->getNext() == nullptr)//base case. the last node's next is always a nullptr
    {
        head_ptr_ = current_first_ptr;//assign the last node as new head_ptr_
        
    }
    else
    {
        invertRest(current_first_ptr->getNext());//recusively traverses list
        /* after getting to end of list and assigning new head_ptr_, access the next node and set next_ to current node */
